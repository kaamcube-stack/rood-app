...
function PhoneField({
  ...
  return (
    <View>
      <View style={[fi.box, !editable && fi.boxDisabled, focused && fi.boxFocused, !!error && fi.boxError]}>
        ...
        <View style={[fi.inputRow, { gap: 10 }]}>
          ...
          <TextInput
            style={{ flex: 1, ...typography.bodyLarge, color: colors.textPrimary, padding: 0 }}
            value={phone}
...
export default function SignupScreen() {
  ...
  const { phone: routePhone = '' } = useRoute<Route>().params ?? {};
  ...
  const phoneEditable = !routePhone;
  ...
  function validate() {
    ...
    if (phoneEditable && !/^\d{10}$/.test(phone)) errs.phone = 'Please enter a valid 10-digit mobile number';
    setErrors(errs);
  ...
  const canSubmit = termsChecked && !!firstName && !!surname && !!email && (phoneEditable ? phone.length === 10 : true);
        ...
        <View style={s.sheet}>
          ...
          >
            ...
            <View style={{ gap: 12 }}>
              ...
              <PhoneField
                phone={phone}
                onChangePhone={(t) => { setPhone(t.replace(/\D/g, '')); clearErr('phone'); }}
